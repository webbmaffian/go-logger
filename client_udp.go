...
func NewUDPClient(ctx context.Context, opt ClientTCPOptions) Transport {
	return &udpClient{
		ctx:  ctx,
		opt:  opt,
		time: time.Now,
...
func (c *udpClient) SetNowFunc(f func() time.Time) {
	c.time = f
}
...
loop:
	for {
		if c.conn == nil {
			...
			c.conn, err = c.dialer.DialContext(c.ctx, "udp", c.Address())
